...
abstract class ItemAnimatorChangeDecorator(val decorated: ItemAnimator) : ItemAnimator() {
	init {
	...
	override fun onAnimationFinished(viewHolder: ViewHolder)
			= decorated.onAnimationFinished(viewHolder)

	override fun onAnimationStarted(viewHolder: ViewHolder)
			= decorated.onAnimationStarted(viewHolder)
